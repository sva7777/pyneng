...
def insert_records_into_dhcp(conn, switch_name, records):
    ...
    sql_insert_dhcp= "INSERT OR IGNORE INTO dhcp VALUES (?,?,?,?,?,?) "
    sql_update_dhcp = "UPDATE dhcp SET ip = ?, vlan = ? , interface = ?, switch= ? , active = ? WHERE mac =?"
    ...
    for line in records:
        try:
            ...
            to_update = (line['ip'], line['vlan'], line['intf'],switch_name, 1 , line['mac'])
            
            conn.execute(sql_insert_dhcp, to_insert )
            conn.execute(sql_update_dhcp, to_update )
