...
def show_table_data_helper(conn, table_name, column_name =None , value =None):
    sql_query = "SELECT * from dhcp"
    if column_name != None :
        sql_query = sql_query +" WHERE " + column_name +" = ?"
        res = conn.execute(sql_query, (value,) )
    else:
...
def show_table_data(conn, table_name, column_name = None, value = None):
    ...
    show_table_data_helper(conn, table_name, column_name, value)
